package com.aronagent.connectionService.repository;
...
public interface PersonRepository extends Neo4jRepository<Person, Long> {
    ...
    @Query("""
            MATCH (personA:Person)-[:CONNECTED_TO]-(personB:Person)
                  ...
                  -[:CONNECTED_TO]-(personD:Person)
            WHERE personA.userId = $userId
            AND personD <> personA
            AND NOT (personA)-[:CONNECTED_TO]-(personD)
            AND NOT (personA)-[:CONNECTED_TO]-()-[:CONNECTED_TO]-(personD)
    ...
    RETURN p
    """)
    List<Person> findAllPersonsExceptMe(Long userId);
}
